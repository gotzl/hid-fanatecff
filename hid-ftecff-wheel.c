#include <linux/module.h>
#include <linux/hid.h>
#include <linux/leds.h>
#include <linux/led-class-multicolor.h>

#include "hid-ftec.h"

#define FTEC_WHEEL_SINGLE_MC 0x1
#define FTEC_WHEEL_FLAG_MC 0x2

#if IS_REACHABLE(CONFIG_LEDS_CLASS)
static void ftec_set_leds(struct hid_device *hid, u16 leds);
#endif

#if IS_REACHABLE(CONFIG_LEDS_CLASS_MULTICOLOR)
static void ftec_wheel_set_led_mc(struct hid_device *hid, u16 led_state_mc[],
				  u8 n_leds, bool rpm_leds)
{
	u8 *buf = kmalloc_array(FTEC_TUNING_REPORT_SIZE, sizeof(u8),
				GFP_KERNEL | __GFP_ZERO);
	if (!buf) {
		hid_err(hid, "ftec_wheel_set_led_mc: Unable to malloc array.");
		return;
	}

	/* FIXME: switch between LED and FLAG leds */
	int j, ret, offset = rpm_leds ? 0 : 9;

	buf[0] = 0xff;
	buf[1] = 0x01;

	buf[2] = rpm_leds ? 0x00 : 0x01;

	for (j = 0; j < n_leds; j++) {
		buf[3 + (2 * j)] = led_state_mc[offset + j] >> 8;
		buf[3 + (2 * j) + 1] = led_state_mc[offset + j] & 0xff;
	}
	ret = hid_hw_output_report(hid, &buf[0], FTEC_TUNING_REPORT_SIZE);
	kfree(buf);
}

static int ftec_wheel_led_mc_set_brightness(struct led_classdev *cdev,
					    enum led_brightness brightness)
{
	struct device *dev = cdev->dev->parent->parent;
	struct hid_device *hid = to_hid_device(dev);
	struct ftec_drv_data *drv_data = hid_get_drvdata(hid);
	struct led_classdev_mc *mc_cdev = lcdev_to_mccdev(cdev);
	uint8_t red, green, blue;
	int j, n;
	u16 led_state_mc[MAX_LEDS];

	if (!drv_data) {
		hid_err(hid, "Device data not found.");
		return -1;
	}

	for (j = 0; j < MAX_LEDS && j < drv_data->wheel.wheel->n_leds; j++) {
		if (mc_cdev == drv_data->wheel.led_mc[j]) {
			led_mc_calc_color_components(mc_cdev, brightness);
			red = mc_cdev->subled_info[0].brightness;
			green = mc_cdev->subled_info[1].brightness;
			blue = mc_cdev->subled_info[2].brightness;

			if (drv_data->wheel.wheel->flags &
			    FTEC_WHEEL_SINGLE_MC) {
#if IS_REACHABLE(CONFIG_LEDS_CLASS)
				u16 leds = ((green & 0x1) << 8) |
					   ((red & 0x1) << 4) | (blue & 0x1);
				ftec_set_leds(hid, leds);
#endif
			} else {
				/* BGR565 encoding */
				drv_data->wheel.led_state_mc[j] =
					((u16)(blue >> (8 - 5)) << (6 + 5)) |
					((u16)(green >> (8 - 6)) << 5) |
					(u16)(red >> (8 - 5));
				memcpy(led_state_mc,
				       drv_data->wheel.led_state_mc,
				       sizeof(led_state_mc));

				/* FIXME: switch between RPM and FLAG leds */
				n = j < 9 ? 9 : 6;
				ftec_wheel_set_led_mc(hid, led_state_mc, n,
						      j < 9);
			}
			return 0;
		}
	}
	return -1;
}

static int
ftec_wheel_init_multicolor(struct hid_device *hid,
			   struct ftec_wheel_classdev *ftec_wheel_cdev, int i)
{
	struct mc_subled *mc_led_info;
	struct led_classdev_mc *led;
	struct led_classdev *led_cdev;
	int ret;

	mc_led_info = devm_kmalloc_array(ftec_wheel_cdev->dev, 3,
					 sizeof(*mc_led_info),
					 GFP_KERNEL | __GFP_ZERO);
	if (!mc_led_info)
		return -ENOMEM;

	mc_led_info[0].color_index = LED_COLOR_ID_RED;
	mc_led_info[1].color_index = LED_COLOR_ID_GREEN;
	mc_led_info[2].color_index = LED_COLOR_ID_BLUE;

	led = devm_kzalloc(ftec_wheel_cdev->dev, sizeof(struct led_classdev_mc),
			   GFP_KERNEL);
	if (!led)
		return -ENOMEM;

	led->subled_info = mc_led_info;
	led->num_colors = 3;

	led_cdev = &led->led_cdev;
	/* FIXME: hardcoded switch bitween RPM LEDs strip above the display,
	 * and FLAG LEDs left/right of the display, found eg on F1 steering wheels */
	led_cdev->name = devm_kasprintf(
		ftec_wheel_cdev->dev, GFP_KERNEL, "%s:%04x:RGB:%s%i",
		dev_name(&hid->dev), ftec_wheel_cdev->wheel->id,
		i < 9 ? "RPM" : (i < 12 ? "RFLAG" : "LFLAG"),
		i < 9 ? i + 1 : (i % 3) + 1);
	if (!led_cdev->name)
		return -ENOMEM;

	led_cdev->brightness = 255;
	led_cdev->max_brightness = 255;
	if (ftec_wheel_cdev->wheel->flags & FTEC_WHEEL_SINGLE_MC) {
		led_cdev->brightness = 1;
		led_cdev->max_brightness = 1;
	}
	led_cdev->brightness_set_blocking = ftec_wheel_led_mc_set_brightness;

	ret = led_classdev_multicolor_register(ftec_wheel_cdev->dev, led);
	if (ret < 0) {
		hid_err(hid, "Cannot register multicolor LED device\n");
		return ret;
	}

	ftec_wheel_cdev->led_mc[i] = led;
	return 0;
}
#endif

#if IS_REACHABLE(CONFIG_LEDS_CLASS)
static void ftec_set_leds(struct hid_device *hid, u16 leds)
{
	struct ftec_drv_data *drv_data;
	unsigned long flags;
	s32 *value;

	drv_data = hid_get_drvdata(hid);
	if (!drv_data) {
		hid_err(hid, "Private driver data not found!\n");
		return;
	}

	spin_lock_irqsave(&drv_data->report_lock, flags);

	value = drv_data->report->field[0]->value;

	value[0] = 0xf8;
	value[1] = 0x09;
	value[2] = 0x08;
	value[3] = (leds >> 8) & 0xff;
	value[4] = leds & 0xff;
	value[5] = 0x00;
	value[6] = 0x00;

	send_report_request_to_device(drv_data);
	spin_unlock_irqrestore(&drv_data->report_lock, flags);
}

int _ftec_led_update_state(struct led_classdev *led_cdev,
			   enum led_brightness value,
			   struct led_classdev *leds[], int n, u16 led_state)
{
	int i, state = 0;
	for (i = 0; i < MAX_LEDS && i < n; i++) {
		if (led_cdev != leds[i])
			continue;
		state = (led_state >> i) & 1;
		if (value == LED_OFF && state) {
			led_state &= ~(1 << i);
		} else if (value != LED_OFF && !state) {
			led_state |= 1 << i;
		}
		break;
	}
	return led_state;
}

static void ftec_wheel_led_set_brightness(struct led_classdev *led_cdev,
					  enum led_brightness value)
{
	struct device *dev = led_cdev->dev->parent->parent;
	struct hid_device *hid = to_hid_device(dev);
	struct ftec_drv_data *drv_data = hid_get_drvdata(hid);

	if (!drv_data) {
		hid_err(hid, "Device data not found.");
		return;
	}

	drv_data->wheel.led_state = _ftec_led_update_state(
		led_cdev, value, drv_data->wheel.led,
		drv_data->wheel.wheel->n_leds, drv_data->wheel.led_state);
	ftec_set_leds(hid, drv_data->wheel.led_state);
}

static enum led_brightness
ftec_wheel_led_get_brightness(struct led_classdev *led_cdev)
{
	struct device *dev = led_cdev->dev->parent->parent;
	struct hid_device *hid = to_hid_device(dev);
	struct ftec_drv_data *drv_data = hid_get_drvdata(hid);
	int i, value = 0;

	if (!drv_data) {
		hid_err(hid, "Device data not found.");
		return LED_OFF;
	}

	for (i = 0; i < MAX_LEDS && i < drv_data->wheel.wheel->n_leds; i++) {
		if (led_cdev == drv_data->wheel.led[i]) {
			value = (drv_data->wheel.led_state >> i) & 1;
			break;
		}
	}

	return value ? LED_ON : LED_OFF;
}

static int ftec_wheel_init_led(struct hid_device *hid,
			       struct ftec_wheel_classdev *ftec_wheel_cdev,
			       int i, int j)
{
	struct led_classdev *led;
	int ret;

	led = devm_kzalloc(ftec_wheel_cdev->dev, sizeof(struct led_classdev),
			   GFP_KERNEL);
	if (!led)
		return -ENOMEM;

	led->name = devm_kasprintf(ftec_wheel_cdev->dev, GFP_KERNEL,
				   "%s:%04x:RPM%d", dev_name(&hid->dev),
				   ftec_wheel_cdev->wheel->id, i + 1);
	if (!led->name)
		return -ENOMEM;

	led->brightness = LED_OFF;
	led->max_brightness = LED_ON;
	led->brightness_get = ftec_wheel_led_get_brightness;
	led->brightness_set = ftec_wheel_led_set_brightness;

	ret = led_classdev_register(ftec_wheel_cdev->dev, led);
	if (ret < 0) {
		hid_err(hid, "Cannot register LED device.");
		return ret;
	}

	ftec_wheel_cdev->led[j] = led;
	return 0;
}
#endif

static void ftec_wheel_deinit_leds(struct ftec_wheel_classdev *ftec_wheel_cdev)
{
#if IS_REACHABLE(CONFIG_LEDS_CLASS)
	int i;

	for (i = 0; i < MAX_LEDS; i++) {
#if IS_REACHABLE(CONFIG_LEDS_CLASS_MULTICOLOR)
		if (ftec_wheel_cdev->led_mc[i])
			led_classdev_multicolor_unregister(
				ftec_wheel_cdev->led_mc[i]);
		ftec_wheel_cdev->led_mc[i] = NULL;
#endif
		if (ftec_wheel_cdev->led[i])
			led_classdev_unregister(ftec_wheel_cdev->led[i]);
		ftec_wheel_cdev->led[i] = NULL;
	}
#endif
}

static int ftec_wheel_init_leds(struct hid_device *hid,
				struct ftec_wheel_classdev *ftec_wheel_cdev)
{
#if IS_REACHABLE(CONFIG_LEDS_CLASS)
	int i;

	for (i = 0; i < MAX_LEDS && i < ftec_wheel_cdev->wheel->n_leds; i++) {
		if (ftec_wheel_cdev->wheel->flags &
		    (FTEC_WHEEL_SINGLE_MC | FTEC_WHEEL_FLAG_MC)) {
#if IS_REACHABLE(CONFIG_LEDS_CLASS_MULTICOLOR)
			if (ftec_wheel_init_multicolor(hid, ftec_wheel_cdev, i))
				goto err;
#endif
		} else {
			// Note: order of LEDs seems reversed on F1 IS rim
			if (ftec_wheel_init_led(hid, ftec_wheel_cdev, i,
						ftec_wheel_cdev->wheel->n_leds -
							i - 1))
				goto err;
		}
	}
	return 0;
err:
	ftec_wheel_deinit_leds(ftec_wheel_cdev);
	return -1;
#else
	return 0;
#endif
}

static ssize_t ftec_wheel_name_show(struct device *dev,
				    struct device_attribute *attr, char *buf)
{
	struct ftec_drv_data *drv_data =
		hid_get_drvdata(to_hid_device(dev->parent));
	return scnprintf(buf, PAGE_SIZE, "%s\n", drv_data->wheel.wheel->name);
}
static DEVICE_ATTR(name, S_IRUSR | S_IRGRP | S_IROTH, ftec_wheel_name_show,
		   NULL);

struct class *ftec_wheel_class;

static const struct ftec_wheel wheels[] = {
	{ CSL_STEERING_WHEEL_P1_V2_ID, "CSL Steering Wheel P1 V2 / BMW",
	  FTEC_WHEEL_SINGLE_MC, 1 },
	{ CSL_ELITE_STEERING_WHEEL_WRC_ID, "CSL Elite Steering Wheel WRC",
	  FTEC_WHEEL_SINGLE_MC, 1 },
	//	{ CSL_ELITE_STEERING_WHEEL_MCLAREN_GT3_V2_ID,
	//	  "CSL Elite Steering Wheel McLaren GT3", 0, 0 },
	{ CLUBSPORT_STEERING_WHEEL_F1_IS_ID, "ClubSport Steering Wheel F1 IS",
	  0, 9 },
	{ CLUBSPORT_STEERING_WHEEL_FORMULA_V2_ID,
	  "ClubSport Steering Wheel Formula V2", FTEC_WHEEL_FLAG_MC, 15 },
	//	{ PODIUM_STEERING_WHEEL_PORSCHE_911_GT3_R_ID,
	//	  "Podium Steering Wheel Porsche 911 GT3 R", 0, 9 },
	{ PODIUM_BUTTON_MODULE_ENDURANCE_ID, "Podium Button Module Endurance",
	  FTEC_WHEEL_FLAG_MC, 15 },
	{}
};

int ftec_wheel_classdev_register(struct device *parent,
				 struct ftec_wheel_classdev *ftec_wheel_cdev,
				 u16 wheel_id)
{
	struct hid_device *hdev = to_hid_device(parent);
	int j;

	ftec_wheel_cdev->dev = device_create(ftec_wheel_class, parent, 0, NULL,
					     "%s", dev_name(&hdev->dev));
	if (IS_ERR(ftec_wheel_cdev->dev)) {
		hid_err(hdev, "error creating ftec_wheel device\n");
		ftec_wheel_cdev->dev = NULL;
		return -1;
	}

	ftec_wheel_cdev->wheel = NULL;
	for (j = 0; wheels[j].id != 0; j++) {
		if (wheels[j].id == wheel_id) {
			ftec_wheel_cdev->wheel = &wheels[j];
			break;
		}
	}

	if (!ftec_wheel_cdev->wheel) {
		hid_err(hdev, "unknown wheel_id 0x%04x\n", wheel_id);
		goto err_unregister;
	}

	if (ftec_wheel_init_leds(hdev, ftec_wheel_cdev)) {
		hid_err(hdev, "error init leds\n");
		goto err_unregister;
	}

	if (device_create_file(ftec_wheel_cdev->dev, &dev_attr_name)) {
		hid_err(hdev, "error creating sysfs interface for 'name'\n");
		goto err_deinit_leds;
	}

	return 0;

err_deinit_leds:
	ftec_wheel_deinit_leds(ftec_wheel_cdev);
err_unregister:
	device_unregister(ftec_wheel_cdev->dev);
	ftec_wheel_cdev->dev = NULL;
	ftec_wheel_cdev->wheel = NULL;
	return -1;
}

void ftec_wheel_classdev_unregister(struct ftec_wheel_classdev *ftec_wheel_cdev)
{
	device_remove_file(ftec_wheel_cdev->dev, &dev_attr_name);
	ftec_wheel_deinit_leds(ftec_wheel_cdev);
	device_unregister(ftec_wheel_cdev->dev);
	ftec_wheel_cdev->dev = NULL;
	ftec_wheel_cdev->wheel = NULL;
}
